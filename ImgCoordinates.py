...
class PanZoomWindow(object):
    ...
    def __init__(self, img, windowName = 'PanZoomWindow', onLeftClickFunction = None):
        ...
        cv2.createTrackbar(self.C_TRACKBAR_NAME, self.WINDOW_NAME, 0, 255, self.onCTrackbarMove)
...
#            print("zoomFactor: %s"%zoomFactor)
            ...
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                print("Pixel Coordinates are %s"%coordsInFullImage)
                ...
                cv2.circle(myImage, (coordsInFullImage[1], coordsInFullImage[0]), 1, self.color, -1)
                self.redrawImage()
    ...
    def onCTrackbarMove(self,tickPosition):
        print (abs(tickPosition * 6 - 3) - 1, 2 - abs(tickPosition * 6 - 2),2 - abs(tickPosition * 6 - 4))
        self.color = (int(tickPosition/2),int(tickPosition/3),int(tickPosition))
...
if __name__ == "__main__":
    image='2020-08-29T01-42-31.570Z.png' ### chose the image name, you want to display here
    path = ("C:\\Users\\syahi\\OneDrive - studentupmedumy.onmicrosoft.com\\Satellite Technology\\Assignments\\Semester project\\")  ## please insert here the path to your images!

    path_end= '\\Karambunai\\'
    infile = path+path_end+image

    myImage = cv2.imread(infile,cv2.IMREAD_ANYCOLOR)
    window = PanZoomWindow(myImage, image)
    key = -1
